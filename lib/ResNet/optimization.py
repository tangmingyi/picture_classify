...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu=False, freeze=-1):
    ...
    if freeze != -1:
        ...
        # tvars.extend(attention_layer_7_var)
        # tvars.extend(attention_layer_6_var)
        # tvars.extend(attention_layer_5_var)
        # tvars.extend(attention_layer_4_var)
        # tvars.extend(attention_layer_3_var)
        # tvars.extend(attention_layer_2_var)
        # tvars.extend(attention_layer_1_var)
        # tvars.extend(rnn_var)
        tvars.extend(full_connection_var)
        # tvars.extend(pooler)
        tvars.extend(match_lay)
        tvars.extend(tmy)
        tf.logging.info("will train variable")
        for var in tvars:
